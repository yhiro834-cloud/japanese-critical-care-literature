...
tags: [glucose, insulin, hypoglycemia, icu]
related: [../Hyperglycemic_Crises/DKA_HHS.md, ../../14_Nutrition/README.md]
ssot: true
---

# Glycemic Management in Critical Illness

## 0. まず覚える

重症患者の血糖管理は、正常値へ厳密に近づけることではなく、**持続する高血糖を安全に管理し、低血糖と大きな変動を防ぐこと**が目的である。insulin必要量は栄養、steroid、循環、腎肝機能、体温で急変する。

**簡単に言うと：** 血糖値だけでなく、「どこから糖が入り、どれだけinsulinが入り、患者の代謝がどう変わったか」を同じ時間軸で見る。

| 用語 | 意味 | 実践上のポイント |
|---|---|---|
| persistent hyperglycemia | 一過性でなく持続する高血糖 | SCCM 2024では成人ICUで180 mg/dL以上が持続すればprotocol開始を推奨 |
| hypoglycemia | 血糖が低下し脳などへ必要なglucoseが不足する状態 | 症候性・重症疑いでは確認検査を待って治療を遅らせない |
| glycemic variability | 血糖の振れ幅・変動 | 平均値が同じでも急変や大きな変動は危険を示す |
| POC glucose | bedside機器による迅速血糖測定 | shock、末梢循環不良、浮腫、干渉物質では誤差に注意 |
| CGM | 持続glucose monitor | trend補助であり、ICUでの投与判断は施設validationに従う |
| basal overlap | 持続静注から皮下注へ移る際に基礎insulin作用を重ねること | insulin作用の空白による再高血糖・ketosisを防ぐ |

**新人看護師の到達点：** 血糖値、検体部位、insulin濃度・速度、全glucose source、栄養、steroid、腎代替療法、次回測定時刻を照合できる。栄養中断・line変更・予想外の値ではprotocolを再確認し、低血糖を治療後に再測定する。

> **報告例：** 「経腸栄養が処置のため中断されましたが、insulin infusionは継続中です。直近血糖は急速に低下しています。低血糖予防、測定間隔、insulin/dextrose調整を直ちに確認してください。」

**ベテラン向け深掘り：** capillary POCの誤差、line contamination、CGM lagを考慮し、異常値を臨床像と照合する。DKA/HHS、妊娠、小児、長期高血糖、栄養変更、CRRT、steroid taperでは一般ICU targetを機械的に適用しない。

## Goal: safe control, not a single perfect number
...
- 明確な開始/停止、target range、測定頻度、rate adjustment、nutrition interruption、hypoglycemia rescue
- enteral/parenteral nutrition、dextrose、steroid、dialysis/CRRT、procedure/transportを同じtimelineで管理
...
- 2026-08-12: V2導入、持続高血糖・血糖変動・POC/CGM等の用語、新人の照合・報告、測定限界を追加。SCCM 2024公式指針を再確認。
