---
title: "Respiratory Support — 30-minute Slide Ready"
status: review-needed
created: 2026-08-11
last_updated: 2026-08-12
evidence_reviewed: 2026-08-12
next_review: 2027-02-11
owners: []
reviewers: [Codex evidence review]
tags: [learning-artifact]
related: []
ssot: false
---

# Respiratory Support — 30-minute Slide Ready

## Learning Objectives

1. 酸素療法から侵襲的換気までのescalationを患者所見で説明する。
2. ventilator pressure/waveformをmechanicsに分解する。
3. ARDS肺保護とliberationの安全点を説明する。

## Suggested Slide Structure

1. 酸素はsupport：DO₂とSpO₂の違い
2. escalation ladderと各deviceの役割
3. HFNC/NIV failureを見逃さない
4. equation of motion：pressureの内訳
5. Ppeak/Pplat/PEEPtotal/driving pressure
6. waveformとasynchrony
7. ARDS：baby lungとVILI
8. strong vs conditional recommendations
9. prone nursing checklist
10. SBT ≠ extubation readiness
11. 症例discussion
12. take-home messages

## Key Figures Needed

- [人工呼吸器の図解クイックガイド](../02_Breathing/Mechanical_Ventilation/VENTILATOR_VISUAL_QUICK_GUIDE.md)（初学者・スマートフォン向け）
- 呼吸support escalationと再評価loop（未作成）
- [気道内圧の構成](../../assets/ventilation/airway_pressure_components.svg)（作成済み）
- [PpeakとPplatの比較](../../assets/ventilation/peak_vs_plateau_pressure.svg)（作成済み）
- [正常時間波形の基本](../../assets/ventilation/normal_time_waveforms.svg)（作成済み）
- [auto-PEEPを疑う呼気流波形](../../assets/ventilation/auto_peep_waveform.svg)（作成済み）
- ARDS heterogeneous lung概念図（未作成）
- liberation decision tree（未作成）

## Speaker Notes

各slideで「患者の何を見るか」「悪化なら次に何をするか」を質問する。数値cutoffを唯一の正解として教えない。

## References for Slides

- ERS HFNC guideline 2022, DOI 10.1183/13993003.01574-2021
- ERS/ATS NIV guideline 2017, DOI 10.1183/13993003.02426-2016
- ATS ARDS update 2024, DOI 10.1164/rccm.202311-2011ST
- AARC Patient-Ventilator Assessment guideline 2024, PMID 39048148
- ATS/CHEST liberation guideline 2017, DOI 10.1164/rccm.201610-2075ST
