...
J-STAGEで公式公開されているPDFは、1回最大3件、10秒以上の間隔、1件20MB以下という上限で一時取得します。PDF本体はGitHubへ保存せず、文字抽出とEvidence Card生成後に削除します。過去文献も未確認分から少しずつ処理します。J-STAGEの規約が禁止する大量ダウンロードを避けるため、一括処理は行いません。画像PDFや文字抽出に失敗したPDFは「PDF取得・解析不可」と記録し、内容を推測しません。

設定は `ENABLE_PDF_REVIEW`、`PDF_REVIEW_LIMIT`、`PDF_REQUEST_INTERVAL`、`PDF_MAX_MB` で変更できます。
